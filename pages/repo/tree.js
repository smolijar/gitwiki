...
import { front, api, construct } from '../../src/endpoints';
...
class Tree extends React.Component {
  ...
  static async getInitialProps({ req, query, store }) {
    ...
    const response = await fetchApi(construct(api.tree)(query), { req });
                ...
                this.props.repo.tree.map((item) => {
                  ...
                  return (
                    <Menu.Item key={item.name}>
                      <Link href={{ pathname: '/repo/tree', query: entryRepo }} as={construct(front.tree)(entryRepo)}>
