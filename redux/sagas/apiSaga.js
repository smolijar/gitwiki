...
import { api, construct } from '../../src/endpoints';


export function* fetchTree(action) {
  const data = yield fetchApi(construct(api.tree)(action.data));
  yield put(actions.repo.setTree(data));
}

export function* fetchRefs(action) {
  const data = yield fetchApi(construct(api.refs)(action.data));
  yield put(actions.repo.setRefs(data));
}

export function* fetchUser(action) {
  const data = yield fetchApi(construct(api.user)(action.data));
